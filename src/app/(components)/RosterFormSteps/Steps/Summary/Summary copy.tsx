...
export function Summary() {
  ...
  const [charactersInfo, setCharactersInfo] = useState<any>(null);
  const [salesEventsInfo, setSalesEventsInfo] = useState<any>(null);
  const [leadershipInfo, setLeadershipInfo] = useState<any>(null);
  const [commentsInfo, setCommentsInfo] = useState<any>(null);
              ...
              {charactersInfo.characters.map(
                (character: any, index: number) => (
                  <p key={index} className="text-gray-300">
                    {index + 1} Choice: {character.role} - {character.class} -{" "}
                    {character.spec}
                  </p>
                )
              )}
